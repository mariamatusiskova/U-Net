...
# A TensorBoard utility for logging.
...
# Set Dataset
# Custom dataset class instances for training and validation data
train_dataset = Dataset(imgs_dir=TRAIN_IMGS_DIR, labels_dir=TRAIN_LABELS_DIR, transform=train_transform)
train_loader = DataLoader(train_dataset, batch_size=cfg.BATCH_SIZE, shuffle=True, num_workers=0)
# Utility that loads the dataset in batches. shuffle=True is used for training to ensure that the model doesn't see the data in the same order each epoch.
...
for epoch in range(start_epoch + 1, num_epochs + 1):
    ...
    for batch_idx, data in enumerate(train_loader, 1):
        ...
        # input images are passed through the model to get predictions.
        output = net(img)

        # Backward Propagation
        # Computes the loss, backpropagates the gradients, and updates the model parameters.
        optim.zero_grad()
    ...
    # Validation loop (No Back Propagation)
    with torch.no_grad():
        net.eval()
        val_loss_arr = list()
